...
static void YUYV_to_JPEG(const uint8_t *input, const int input_width, const int input_height,
						 const int stride, const int output_width, const int output_height,
...
static void YUV420_to_JPEG_fast(const uint8_t *input, const int width, const int height,
								const int stride, const int quality, const unsigned int restart,
...
static void YUV420_to_JPEG(const uint8_t *input, const int input_width, const int input_height,
						   const int stride, const int output_width, const int output_height,
						   const int quality, const unsigned int restart, uint8_t *&jpeg_buffer,
						   jpeg_mem_len_t &jpeg_len)
{
	...
	{
		YUV420_to_JPEG_fast(input, input_width, input_height, stride, quality, restart, jpeg_buffer,
							jpeg_len);
		return;
	}
						...
						const unsigned int restart, uint8_t *&jpeg_buffer, jpeg_mem_len_t &jpeg_len)
{
	if (pixel_format == libcamera::formats::YUYV)
		YUYV_to_JPEG(input, input_width, input_height, stride, output_width, output_height, quality,
					 restart, jpeg_buffer, jpeg_len);
	else if (pixel_format == libcamera::formats::YUV420)
		YUV420_to_JPEG(input, input_width, input_height, stride, output_width, output_height,
					   quality, restart, jpeg_buffer, jpeg_len);
	else
		throw std::runtime_error("unsupported YUV format in JPEG encode");
}

static void create_exif_data(PixelFormat const &pixel_format, std::vector<void *> const &mem, int w,
							 int h, int stride, ControlList const &metadata,
							 ...
							 jpeg_mem_len_t &thumb_len)
{
	...
	try
	{
		...
		for (; q > 0; q -= 5)
		{
			YUV_to_JPEG(pixel_format, (uint8_t *)(mem[0]), w, h, stride, options.thumb_width,
...
void jpeg_save(std::vector<void *> const &mem, int w, int h, int stride,
			   PixelFormat const &pixel_format, ControlList const &metadata,
			   ...
			   StillOptions const &options)
{
	...
	try
	{
		...
		create_exif_data(pixel_format, mem, w, h, stride, metadata, cam_name, options, exif_buffer,
		...
		jpeg_mem_len_t jpeg_len;
		YUV_to_JPEG(pixel_format, (uint8_t *)(mem[0]), w, h, stride, w, h, options.quality,
