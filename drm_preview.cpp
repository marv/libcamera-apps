...
void DrmPreview::findCrtc()
{
	...
	if (!conId_)
	{
		...
		for (i = 0; i < res->count_connectors; i++)
		{
			...
			if (options_.verbose)
				std::cout << "Connector " << con->connector_id << " (crtc "
						  << (crtc ? crtc->crtc_id : 0) << "): type " << con->connector_type << ", "
							...
							unsigned int stride, Buffer &buffer)
{
	...
	if (drmModeAddFB2(drmfd_, width, height, out_fourcc_, bo_handles, pitches, offsets,
					  &buffer.fb_handle, 0))
...
void DrmPreview::Show(int fd, size_t size, int width, int height, int stride)
{
	...
	if (drmModeSetPlane(drmfd_, planeId_, crtcId_, buffer.fb_handle, 0, x_off + x_, y_off + y_, w,
						h, 0, 0, buffer.width << 16, buffer.height << 16))
